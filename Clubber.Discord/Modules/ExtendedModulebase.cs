	...
	where T : SocketCommandContext
{
	...
	protected async Task InlineReplyAsync(string message, bool ping = false)
		=> await ReplyAsync(message, allowedMentions: ping ? null : AllowedMentions.None, messageReference: new MessageReference(Context.Message.Id));
	...
	protected async Task<Result<SocketGuildUser>> FoundOneUserFromName(string name)
	{
		...
		await ReplyAsync(embed: EmbedHelper.MultipleMatches(userMatches, name), allowedMentions: AllowedMentions.None, messageReference: new MessageReference(Context.Message.Id));
